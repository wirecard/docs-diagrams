...
\node [split, text height=.6cm, text depth=.4cm, right of=request1] (response1) {\nodepart [white]{one} 2 capture (Response)
...
\node [split,right of=request2] (response2) {\nodepart [white]{one} 4 refund-capture (Response)
						\nodepart {two}  $<$requested-amount \\ currency="EUR"$>$20.00$<$/ \\ requested-amount$>$ };
\node [outcome, text height=0.9cm, text depth=1.5cm, right of=response2, white] (outcome) {Merchant returns funds to consumer's account.};
