...
\node [split, text height=0.5cm, text depth=0.5cm, right of=request1] (response1) {\nodepart [white]{one} 2 capture (Response)
...
\node [split,right of=request2] (response2) {\nodepart [white]{one} 4 void-capture (Response)
						\nodepart {two}  $<$requested-amount \\ currency="EUR"$>$20.00$<$/ \\ requested-amount$>$ };
\node [outcome, text height=1.2cm, text depth=1.2cm, right of=response2, white] (outcome) {Funds not transferred from consumer's account};
